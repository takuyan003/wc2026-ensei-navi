...
const state = {
  tab: "today",
  stadiumFilter: "all",
  tip: { country: "us", scene: "restaurant", pctIndex: 1, amount: "" },
  mode: null,
};
...
// ===== チップ計算タブ =====
function renderTip() {
  const { country, scene, pctIndex, amount } = state.tip;
  ...
  if (sceneData.flat) {
    ...
    const pct = sceneData.pct[pctIndex];
  ...
  return `
    <div class="card">
      ...
      ${sceneData.flat ? "" : `
      <div class="field">
        <label>金額（${guide.currency}・税抜額がベター）</label>
        <input type="number" id="tip-amount" inputmode="decimal" placeholder="0.00" value="${amount}">
      </div>
      <div class="field">
        <label>チップ率</label>
        <div class="seg" id="tip-pct">
          ${sceneData.pct.map((p, i) => `<button data-i="${i}" class="${i === pctIndex ? "active" : ""}">${p}%</button>`).join("")}
        </div>
      </div>`}
      ${resultHtml}
...
document.body.addEventListener("click", e => {
  ...
  if (c) { state.tip.country = c.dataset.c; state.tip.scene = "restaurant"; state.tip.pctIndex = 1; render(); return; }
  const s = e.target.closest("#tip-scene button");
  if (s) { state.tip.scene = s.dataset.s; state.tip.pctIndex = 1; render(); return; }
  const p = e.target.closest("#tip-pct button");
  if (p) { state.tip.pctIndex = +p.dataset.i; render(); return; }
});
