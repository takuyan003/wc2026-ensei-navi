...
const STADIUMS = [
  ...
  { id: "dallas", country: "us", fifaName: "ダラス・スタジアム", known: "AT&T Stadium", city: "アーリントン (TX)", tz: "America/Chicago",
    ...
    depart: "鉄道なし。①公式シャトル（TRE→CentrePort/DFW駅から無料・要チケット）②配車はLot 15(Webb St)の指定乗り場のみ・サージで20マイル$80超も ③アーリントン中心部ホテルはRed Trolley（試合30分後発）。試合直後45〜60分は大渋滞、ピークを外すのが吉" },
  ...
  { id: "monterrey", country: "mx", fifaName: "エスタディオ・モンテレイ", known: "Estadio BBVA", city: "モンテレイ（グアダルペ）", tz: "America/Monterrey",
    ...
    depart: "日本戦は22時KO→深夜帰宅。①地下鉄1号線Exposición駅（徒歩約10分）②Park&Rideシャトル ③Uber/DiDiは終了直後サージ＆待ち長い→入場前に帰りを予約/予約配車を。スタジアム周辺は歩行者規制あり" },
  // カナダ
...
const MODES = {
  ...
  gohome: {
    ...
    steps: [
      ...
      "人混みから少し離れた場所へ移動する",
      "公式シャトル・公共交通の最終便を確認（配車は終了直後サージで高額・指定乗り場のみ）",
      "下のホテル住所を運転手や係員に見せる",
...
// ===== 試合後の帰り方のコツ（混雑・サージ対策・2026-06-12検証済み） =====
const TRANSPORT_TIPS = [
  "🎟️ 入場前に「帰りの手段」を決めておく（配車の予約・シャトル最終便の時刻）",
  "⏰ 試合直後45〜60分は大渋滞。スタジアムで余韻を楽しんでピークを外す",
  "🚕 Uber/Lyft/DiDiは終了直後サージで高額（20マイルで$80超も）。指定ピックアップゾーンからのみ乗車",
  "🚶 渋滞エリアから少し歩いて拾うと、早く安く捕まりやすい",
  "🚌 公式シャトル（Park&Ride）・公共交通は増発される。最終便の時刻だけは必ず確認",
  "👥 同行者やまわりの日本人サポーターと相乗りすれば割り勘で安心",
  "🌙 モンテレイの日本戦は深夜帰宅。予約配車かホテルの送迎手配が安全",
];
